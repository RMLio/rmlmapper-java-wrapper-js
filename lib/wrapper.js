...
class RMLMapperWrapper {
   ...
   * @param {boolean} [burpMode = false] - Whether to run in Burp mode.
   */
  constructor(path, tempFolder, removeTempFolders, stateFolder = tempFolder + "/function_state", stateFolderTimeToLive = 600, javaVMOptions = {}, burpMode = false) {
    this.path = path;
    this.tempFolder = tempFolder;
    this.removeTempFolders = removeTempFolders;
    this.burpMode = burpMode;
   ...
   * @param {string} options.functionStateId - Id for state: if a functions keeps state, users can refer to that state with this id. The default is an empty string.
  ...
  async execute(rml, options = {}) {
    ...
    if (!options.functionStateId) {
      options.functionStateId = '';
    }
    ...
    return new Promise(async (resolve, reject) => {
      ...
      const processDir = path.join(this.tempFolder, '' + ms + '_' + Math.random());
  ...
  async _executeCLI(options) {
    ...
    // trim the options that BURP does not support here and hope for the best
    if (!this.burpMode) {
      execParams.push(`-s`)
  ...
  async _setSourcesInRMLRules(rmlQuads, prefix, sources) {
    return new Promise((resolve, reject) => {
      ...
      store.addQuads(adjustedRMLQuads);
      const sourceNodes = store.getObjects(null, namedNode('http://w3id.org/rml/source'));
      sourceNodes.forEach(sourceNode => {
        const paths = store.getObjects(sourceNode, namedNode('http://w3id.org/rml/path'));
        if (paths.length > 0) {
          store.addQuad(newQuad(sourceNode, namedNode('http://w3id.org/rml/path'), literal(prefix + sources[paths[0].value].path)));
          store.removeQuad(newQuad(sourceNode, namedNode('http://w3id.org/rml/path'), paths[0]))
        }
      });
      adjustedRMLQuads = [];
      for (const quad of store) {
        adjustedRMLQuads.push(quad);
      }
  ...
  _updateTargetsWithLocalFiles(options = {}) {
    ...
    // support for RMLKGC  
    const targetNodes = store.getObjects(null, namedNode('http://w3id.org/rml/target'));

    targetNodes.forEach(targetNode => {
      const voidDataDumps = store.getObjects(targetNode, namedNode('http://rdfs.org/ns/void#dataDump'));
      const dcatDataDumps = store.getObjects(targetNode, namedNode('http://www.w3.org/ns/dcat#dataDump'));
      const filePaths = store.getObjects(targetNode, namedNode('http://w3id.org/rml/path'));
      ...
      if (voidDataDumps.length > 0) {
        ...
        const filename = path.basename(filePaths[0].value);

        let p = dirPrefix + filename;
        targetFiles.push(p);
        p = this._fixPath(p);

        store.addQuad(targetNode, namedNode('http://w3id.org/rml/path'), namedNode('file://' + p));
        store.removeQuad(targetNode, namedNode('http://w3id.org/rml/path'), dcatDataDumps[0]);
      }
  ...
  /**
   * Cleans up function state directories used by RMLMapper. When called each function state
   * directory is checked against `stateFolderTimeToLive`. If it is older than that (in seconds)
   * then that directory is deleted.
   * @returns {Promise<void>}
   * @private
   */
  async _cleanUpStateFolder() {
    const stateSubFolders = await fs.readdir(this.stateFolder);
    for await (const stateSubFolder of stateSubFolders) {
      const completePathToDelete = path.join(this.stateFolder, stateSubFolder);
      const stat = await fs.stat(completePathToDelete);
      ...
      if (timePassed > this.stateFolderTimeToLive) {
        await fs.rm(completePathToDelete, { recursive: true, force: true });
      }
    }
  }
}
